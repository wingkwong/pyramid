...
const { getRuleStore, getInputFileStore, getResultStore, setResultStore } = require('./stores');
...
const _processRule = (rule) => {
    let RESULT_STORE = getResultStore();
...
const _outputRuleResult = () => {
    logger('------------------------------------------------------------------------');
    logger('[INFO] Rule Engine Result Summary');
    logger('------------------------------------------------------------------------');
    const RESULT_STORE = getResultStore();
    RESULT_STORE.forEach(result => {
        logger(`[INFO] Rule ${result.id} (${result.name}) : ${result.result ? 'SUCCESS' : 'FAILURE'}`);
    });
}
